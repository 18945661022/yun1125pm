...
then
    ...
    then
        if [[ $ARCHS == *"armv7"* ]]
        then
            "$PROJECT_DIR/MapFileParser" -format=Clang "$TARGET_TEMP_DIR/$PRODUCT_NAME-LinkMap-$CURRENT_VARIANT-armv7.txt" "$TARGET_BUILD_DIR/$PRODUCT_NAME.app/Data/Managed/SymbolMap-32"
        fi
        if [[ $ARCHS == *"arm64"* ]]
        then
            "$PROJECT_DIR/MapFileParser" -format=Clang "$TARGET_TEMP_DIR/$PRODUCT_NAME-LinkMap-$CURRENT_VARIANT-arm64.txt" "$TARGET_BUILD_DIR/$PRODUCT_NAME.app/Data/Managed/SymbolMap-64"
        fi
    else
